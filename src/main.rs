...
async fn main() {
  ...
  spawn_reader(proxy.handler.stdout.take().unwrap(), &mutex);
  spawn_reader(proxy.handler.stderr.take().unwrap(), &mutex);
...
fn spawn_reader<T: AsyncRead + Send + 'static>(fd: T, mutex: &Arc<Mutex<()>>)
where
  BufReader<T>: Unpin,
{
